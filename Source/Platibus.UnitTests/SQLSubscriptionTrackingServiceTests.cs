...
namespace Platibus.UnitTests
{
    internal class SQLSubscriptionTrackingServiceTests
    {
        ...
        public async Task Handles_Many_Concurrent_Calls_Correctly_Without_Errors()
        {
            ...
            using (var sqlSubscriptionService = new SQLSubscriptionTrackingService(connectionStringSettings))
            {
                await sqlSubscriptionService.Init();
                var tasks = subscriptions
                    .Select(s => sqlSubscriptionService.AddSubscription(s.Topic, s.Subscriber))
                    .ToList();
        ...
        {
            var connectionStringSettings = GetConnectionStringSettings();
            var topicNames = Enumerable.Range(0, 10).Select(i => new TopicName("Topic-" + i));
